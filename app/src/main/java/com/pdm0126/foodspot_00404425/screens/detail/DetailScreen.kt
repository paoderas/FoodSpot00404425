...
fun RestaurantDetailScreen(
    restaurantId: Int, onBack: () -> Unit, viewModel: RestaurantDetailViewModel = viewModel()
) {
    val context = LocalContext.current
    val restaurant by viewModel.restaurant.collectAsState() //Observa los datos
    ...
    Scaffold(topBar = {
        TopAppBar(
            ...
            ), title = { Text(restaurant?.name ?: "Cargando...") }, navigationIcon = {
                IconButton(onBack) {
                    Icon(
        ...
        restaurant?.let { rest ->
            LazyColumn(
                ...
                items(rest.menu) { dish: Dish ->
                    DishCard(
                        dish = dish, onAddToCart = {
                            Toast.makeText(
                                context, "${dish.name} agregado al carrito", Toast.LENGTH_SHORT
                            ).show()
                        })
                }
            }
